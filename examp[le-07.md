...
metadata:
  ...
  labels:
    {{ template "commonLabels" . }}
    version: v1.0.0
spec:
  selector:
    {{ template "commonSelector" dict "Version" "v1.0.0" }}
  template:
    metadata:
      labels:
        {{ template "commonLabels" . }}
        version: v1.0.0
    spec:
      {{ template "commonPodSpec" dict "Image" "microservice:v1.0.0" }}
...
spec:
  selector:
    {{ template "commonSelector" dict "Version" "v1.1.0" }}
  template:
    metadata:
      labels:
        {{ template "commonLabels" . }}
        version: v1.1.0
    spec:
      {{ template "commonPodSpec" dict "Image" "microservice:v1.1.0" }}
